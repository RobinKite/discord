export const blurple = "#5865f2";
export const blurple50 = "#4752c4";
export const green = "#23A559";
export const yellow = "#fee75c";
export const fuchsia = "#eb459e";
export const red = "#ed4245";
const white = "#ffffff";
const darkgreen = { 10: "#08853c", 20: "#026123" };

// common
const buttonBgDefault = blurple;
const buttonBgHover = blurple50;
const buttonBgDisabled = blurple50;

// dark theme
export const darkActive = "#99aab5";
export const darkUsersList = "#2b2d31";
export const darkUserSection = "#232428";
export const darkServerIconBg = "#2c2f33";
export const darkSidebar = "#1E1F22";
export const darkTextLighter = white;
export const darkText = "#dbded1";
export const darkBg = "#23272a";
export const darkChannelName = "#949ba4";
export const darkTooltip = "#111214";

// light theme
export const lightActive = "#e5e5e5";
export const lightUsersList = "#f2f3f5";
export const lightUserSection = "#ebedef";
export const lightSidebar = "#bfbfbf";
export const lightText = "#313338";
export const lightTextDarker = "#060607";
export const lightBg = white;
export const lightChannelName = "#5c5e66";

/** fonts: Uni Sans Font or PT Sans */
export const font = "PT Sans";

export const grey = {
  10: "#b5bac1",
  50: "#f6f8fa",
  100: "#eaeef2",
  200: "#d0d7de",
  300: "#afb8c1",
  400: "#8c959f",
  500: "#6e7781",
  600: "#57606a",
  700: "#424a53",
  800: "#32383f",
  900: "#24292f",
};

export default {
  darkText,
  grey900: grey[900],
  darkTextLighter,
  darkSidebar,
  darkUsersList,
  white,
  blurple,
  buttonBgHover,
  buttonBgDisabled,
  red,
  buttonBgDefault,
  darkUserSection,
  lightText,
  lightUsersList,
  grey,
  green,
  darkgreen,
  darkTooltip,
  darkBg,
};
